...
proc generate {drv_handle} {
    puts "Generating Macros for adc_driver_axiw driver access ... "
    ...
    eval xdefine_include_file $drv_handle "xparameters.h" "ADC_DRIVER_AXIW" "NUM_INSTANCES" ${xparam_config_table}
    eval sg_xdefine_include_file $drv_handle "xparameters.h" "ADC_DRIVER_AXIW" ${addr_config_table}
    # generate adc_driver_axiw_g.c
    eval xdefine_config_file $drv_handle "adc_driver_axiw_g.c" "ADC_DRIVER_AXIW" ${config_table}
}
...
proc sg_xdefine_include_file {drv_handle file_name drv_string args} {
    ...
    foreach periph ${periphs} {
        ...
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CREATE    xc_adc_driver_axiw_create"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_RELEASE   xc_adc_driver_axiw_release"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_OPEN      xc_adc_driver_axiw_open"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CLOSE     xc_adc_driver_axiw_close"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_READ      xc_adc_driver_axiw_read"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_WRITE     xc_adc_driver_axiw_write"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_GET_SHMEM xc_adc_driver_axiw_getshmem"
