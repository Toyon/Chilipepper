...
#include "dac_driver_axiw.h"
#include "xparameters.h"
#include "xil_io.h"
#include "xcope.h"

inline xc_status_t xc_dac_driver_axiw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    DAC_DRIVER_AXIW_Config *_config_table = config_table;
...
inline xc_status_t xc_dac_driver_axiw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_dac_driver_axiw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_dac_driver_axiw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_dac_driver_axiw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = Xil_In32((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_dac_driver_axiw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
    Xil_Out32((uint32_t *) addr, value);
    return XC_SUCCESS;
}

xc_status_t xc_dac_driver_axiw_getshmem(xc_iface_t *iface, const char *name, void **shmem)
{
    DAC_DRIVER_AXIW_Config *_config_table = (DAC_DRIVER_AXIW_Config *) iface;

    if (strcmp("dac_en", name) == 0) {
        *shmem = (void *) & _config_table->dac_en;
    }
