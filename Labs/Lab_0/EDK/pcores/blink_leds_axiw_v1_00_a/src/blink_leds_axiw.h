...
#ifndef __XL_BLINK_LEDS_AXIW_H__
#define __XL_BLINK_LEDS_AXIW_H__
...
typedef struct {
    ...
    Xuint16  DeviceId;
    uint32_t  BaseAddr;
} BLINK_LEDS_AXIW_Config;

extern BLINK_LEDS_AXIW_Config BLINK_LEDS_AXIW_ConfigTable[];

// forward declaration of low-level functions
xc_status_t xc_blink_leds_axiw_create(xc_iface_t **iface, void *config_table);
xc_status_t xc_blink_leds_axiw_release(xc_iface_t **iface) ;
xc_status_t xc_blink_leds_axiw_open(xc_iface_t *iface);
xc_status_t xc_blink_leds_axiw_close(xc_iface_t *iface);
xc_status_t xc_blink_leds_axiw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value);
xc_status_t xc_blink_leds_axiw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value);
