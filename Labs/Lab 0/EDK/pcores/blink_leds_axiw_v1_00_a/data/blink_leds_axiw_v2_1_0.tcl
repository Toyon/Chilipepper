...
proc generate {drv_handle} {
    puts "Generating Macros for blink_leds_axiw driver access ... "
    ...
    eval xdefine_include_file $drv_handle "xparameters.h" "BLINK_LEDS_AXIW" "NUM_INSTANCES" ${xparam_config_table}
    eval sg_xdefine_include_file $drv_handle "xparameters.h" "BLINK_LEDS_AXIW" ${addr_config_table}
    # generate blink_leds_axiw_g.c
    eval xdefine_config_file $drv_handle "blink_leds_axiw_g.c" "BLINK_LEDS_AXIW" ${config_table}
}
...
proc sg_xdefine_include_file {drv_handle file_name drv_string args} {
    ...
    foreach periph ${periphs} {
        ...
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CREATE    xc_blink_leds_axiw_create"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_RELEASE   xc_blink_leds_axiw_release"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_OPEN      xc_blink_leds_axiw_open"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CLOSE     xc_blink_leds_axiw_close"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_READ      xc_blink_leds_axiw_read"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_WRITE     xc_blink_leds_axiw_write"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_GET_SHMEM xc_blink_leds_axiw_getshmem"
