...
inline xc_status_t xc_tx_axiw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    TX_AXIW_Config *_config_table = config_table;
...
inline xc_status_t xc_tx_axiw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_tx_axiw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_tx_axiw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_tx_axiw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = Xil_In32((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_tx_axiw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
...
{
    TX_AXIW_Config *_config_table = (TX_AXIW_Config *) iface;

    if (strcmp("tx_done", name) == 0) {
        *shmem = (void *) & _config_table->tx_done;
    } else if (strcmp("tx_en", name) == 0) {
        *shmem = (void *) & _config_table->tx_en;
    } else if (strcmp("clear_fifo", name) == 0) {
        *shmem = (void *) & _config_table->clear_fifo;
    } else if (strcmp("tx_fifo", name) == 0) {
        *shmem = (void *) & _config_table->tx_fifo;
    }
