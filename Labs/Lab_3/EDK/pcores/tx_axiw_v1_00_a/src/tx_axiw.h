...
#ifndef __XL_TX_AXIW_H__
#define __XL_TX_AXIW_H__
...
typedef struct {
    ...
    uint32_t tx_done;
    uint32_t tx_done_n_bits;
    uint32_t tx_done_bin_pt;
    // uint32_t tx_done_attr;
    uint32_t tx_en;
    uint32_t tx_en_n_bits;
    uint32_t tx_en_bin_pt;
    // uint32_t tx_en_attr;
    uint32_t clear_fifo;
    uint32_t clear_fifo_n_bits;
    uint32_t clear_fifo_bin_pt;
    // uint32_t clear_fifo_attr;
    uint32_t tx_fifo;
    uint32_t tx_fifo_percentfull;
    uint32_t tx_fifo_full;
    uint32_t tx_fifo_n_bits;
    uint32_t tx_fifo_bin_pt;
    uint32_t tx_fifo_depth;
    // uint32_t tx_fifo_attr;
    // XPS parameters
    Xuint16  DeviceId;
    uint32_t  BaseAddr;
} TX_AXIW_Config;

extern TX_AXIW_Config TX_AXIW_ConfigTable[];

// forward declaration of low-level functions
xc_status_t xc_tx_axiw_create(xc_iface_t **iface, void *config_table);
xc_status_t xc_tx_axiw_release(xc_iface_t **iface) ;
xc_status_t xc_tx_axiw_open(xc_iface_t *iface);
xc_status_t xc_tx_axiw_close(xc_iface_t *iface);
xc_status_t xc_tx_axiw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value);
xc_status_t xc_tx_axiw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value);
xc_status_t xc_tx_axiw_getshmem(xc_iface_t *iface, const char *name, void **shmem);
