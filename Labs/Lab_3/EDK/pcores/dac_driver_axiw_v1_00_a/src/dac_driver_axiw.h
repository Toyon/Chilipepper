...
#ifndef __XL_DAC_DRIVER_AXIW_H__
#define __XL_DAC_DRIVER_AXIW_H__
...
typedef struct {
    ...
    uint32_t dac_en;
    uint32_t dac_en_n_bits;
    uint32_t dac_en_bin_pt;
    // uint32_t dac_en_attr;
    // XPS parameters
    Xuint16  DeviceId;
    uint32_t  BaseAddr;
} DAC_DRIVER_AXIW_Config;

extern DAC_DRIVER_AXIW_Config DAC_DRIVER_AXIW_ConfigTable[];

// forward declaration of low-level functions
xc_status_t xc_dac_driver_axiw_create(xc_iface_t **iface, void *config_table);
xc_status_t xc_dac_driver_axiw_release(xc_iface_t **iface) ;
xc_status_t xc_dac_driver_axiw_open(xc_iface_t *iface);
xc_status_t xc_dac_driver_axiw_close(xc_iface_t *iface);
xc_status_t xc_dac_driver_axiw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value);
xc_status_t xc_dac_driver_axiw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value);
