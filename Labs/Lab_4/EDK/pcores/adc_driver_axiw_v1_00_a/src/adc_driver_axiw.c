...
#include "adc_driver_axiw.h"
#include "xparameters.h"
#include "xil_io.h"
#include "xcope.h"

inline xc_status_t xc_adc_driver_axiw_create(xc_iface_t **iface, void *config_table)
{
    // set up iface
    *iface = (xc_iface_t *) config_table;

#ifdef XC_DEBUG
    ADC_DRIVER_AXIW_Config *_config_table = config_table;
...
inline xc_status_t xc_adc_driver_axiw_release(xc_iface_t **iface) 
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_adc_driver_axiw_open(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_adc_driver_axiw_close(xc_iface_t *iface)
{
    // does nothing
    return XC_SUCCESS;
}

inline xc_status_t xc_adc_driver_axiw_read(xc_iface_t *iface, xc_r_addr_t addr, uint32_t *value)
{
    *value = Xil_In32((uint32_t *) addr);
    return XC_SUCCESS;
}

inline xc_status_t xc_adc_driver_axiw_write(xc_iface_t *iface, xc_w_addr_t addr, const uint32_t value)
{
...
{
    ADC_DRIVER_AXIW_Config *_config_table = (ADC_DRIVER_AXIW_Config *) iface;
