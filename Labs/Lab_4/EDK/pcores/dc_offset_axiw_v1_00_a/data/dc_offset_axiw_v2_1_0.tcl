...
proc generate {drv_handle} {
    puts "Generating Macros for dc_offset_axiw driver access ... "
    ...
    eval xdefine_include_file $drv_handle "xparameters.h" "DC_OFFSET_AXIW" "NUM_INSTANCES" ${xparam_config_table}
    eval sg_xdefine_include_file $drv_handle "xparameters.h" "DC_OFFSET_AXIW" ${addr_config_table}
    # generate dc_offset_axiw_g.c
    eval xdefine_config_file $drv_handle "dc_offset_axiw_g.c" "DC_OFFSET_AXIW" ${config_table}
}
...
proc sg_xdefine_include_file {drv_handle file_name drv_string args} {
    ...
    foreach periph ${periphs} {
        ...
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CREATE    xc_dc_offset_axiw_create"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_RELEASE   xc_dc_offset_axiw_release"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_OPEN      xc_dc_offset_axiw_open"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_CLOSE     xc_dc_offset_axiw_close"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_READ      xc_dc_offset_axiw_read"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_WRITE     xc_dc_offset_axiw_write"
        puts $file_handle "#define XPAR_[string toupper [xget_hw_name ${periph}]]_XC_GET_SHMEM xc_dc_offset_axiw_getshmem"
